...
export class GameScene extends Phaser.Scene {
  ...
  create(): void {
    ...
    const { playerEntity, currentHero } = HeroFactory.createPlayer(
      this,
      worldSize / 2,
      worldSize / 2,
      this.gameState,
    ...
    this.spawnManager = new SpawnManager(
      () => ({
        ...
        vx: this.playerEntity.sprite?.body?.velocity.x ?? 0,
        vy: this.playerEntity.sprite?.body?.velocity.y ?? 0,
      }),
      (def, x, y, scaling) => {
    ...
    CollisionManager.setup({
      ...
      hud: this.hud,
      audio: this.audio,
      getPlayerIframeTimer: () => this.playerIframeTimerMs,
      applyDamageToPlayer: (dmg) => this.applyDamageToPlayer(dmg),
        ...
        if (enemy && !enemy.isExploding) {
          if (enemy.definition?.archetype === 'exploder') {
            ...
            if (this.currentHero?.id === 'hero_markovka') {
              this.heroTraitSystem.onEnemyKilledByMarkovka(this.getTraitCtx());
            }
          }
        }
      }),
  ...
  private applyDamageToPlayer(dmg: number): void {
    ...
    if (sprite) this.hazardSystem.flashSprite(this, sprite, 0xff4444);
  ...
  private triggerPlayerDeath(): void {
    ...
    if (sprite?.active) {
      ...
      if (this.currentHero?.id === 'hero_worm' && this.anims.exists('vypolzok_anim_dead')) {
        sprite.play('vypolzok_anim_dead');
      } else {
        this.tweens.add({ targets: sprite, angle: 90, scaleX: sprite.scaleX * 1.3, scaleY: sprite.scaleY * 0.6, alpha: 0.5, duration: 350, ease: 'Bounce.easeOut' });
      }
    }
  ...
  override update(_time: number, delta: number): void {
    ...
    this.weaponManager.update(delta, {
      ...
      projectilesGroup: this.playerProjectilesGroup,
      enemiesMap: this.enemiesMap,
      flashSprite: (s, c) => this.hazardSystem.flashSprite(this, s, c),
