...
8. **Запрещено** переключать `GameConfig.ts` на `Scale.RESIZE` или `'100%'` — холст обязан использовать `Phaser.Scale.EXPAND` с базой `1280×720` и `roundPixels: false` для обеспечения максимальной чёткости и отсутствия мыла на мобильных Retina экранах.
