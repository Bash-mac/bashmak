...
│   │   ├── weapons/        # 1 файл = 1 оружие с Lv.1-5 (slimeSpit.ts, laceWhip.ts...)
│   │   ├── tomes/          # 1 файл = 1 том (speedKeds.ts, slimeSoda.ts, knockedTooth.ts, leech.ts...)
...
│   │       └── AcidTrailWeapon.ts
│   ├── audio/          # Звуковой движок (Hybrid Audio Sprite & Foley Synth)
│   │   └── AudioManager.ts
...
### 2.8. VFX & Asset Pipeline (`D:\DevTools\ArcadaEffector`)
- **Инструмент**: Автономный редактор частиц Arcaidia Effector (`D:\DevTools\ArcadaEffector`, локальный сервер `http://localhost:5179`).
- **Расширяемость движка**: Если для реализации эффекта не хватает возможностей (шейпы, деформации, турбулентность, модификаторы), разрешено и поощряется дописывать код напрямую в движок редактора (`js/core/`, `js/ui/`).
- **Библиотека эффектов**: JSON-определения в `D:\DevTools\ArcadaEffector\library/*.json`.
- **Экспорт спрайт-атласов**: Запекание текстур в `public/assets/sprites/vfx/`, регистрация анимаций в `BootScene.ts`, спавн через пулы `VfxPool.ts`.
- **Процедурный рендеринг**: Для мгновенных электрических разрядов используется `VfxPool.spawnProceduralLightning()` через пул `Phaser.GameObjects.Graphics` (Zero-Allocation, многослойный WebGL-рендеринг).
...
7. **Запрещено** верстать UI/модалки в абсолютных пикселях под конкретный экран или использовать `if (width < 700)` — весь UI верстается ТОЛЬКО под единый Virtual Viewport (база `1280x720` в Landscape) внутри контейнера с масштабом `Math.min((width * 0.96) / 1280, (height * 0.96) / 720)`. Запрещено зажимать масштаб через `Math.min(1.0, ...)`.
8. **Запрещено** переключать `GameConfig.ts` на `Scale.RESIZE` или `'100%'` — холст обязан использовать `Phaser.Scale.EXPAND` с базой `1280×720` для обеспечения максимальной чёткости и правильного масштабирования на мобильных экранах.
...
5. **HUD Рендеринг (Иммунитет к дрожанию камеры)**: статические элементы интерфейса (`HUD.ts`) создаются как прямые автономные объекты сцены с `setScrollFactor(0).setDepth(9000)` без родительского `Container`. Это исключает субпиксельное перемножение матриц движущейся камеры в WebGL и гарантирует абсолютно стабильный текст без вибраций при беге.

---

## 6. Звуковая и тактильная архитектура (Hybrid Audio & Haptic Engine)

1. **Гибридный саунд-дизайн**:
   - Воспроизведение коротких фоли-семплов (Audio Sprite / predecoded `AudioBuffer`) с автоматическим переключением на многослойный органический процедурный синтез.
   - **Voice Stealing & Channel Limiter**: ограничение одновременных голосов (максимум 2–4 голоса на категорию `splat`, `spit`, `zap`, `explosion`) исключает перегрузку шины и клиппинг в плотных ордах.
   - **Micro-Pitch Variation**: детьюн ±10–15% на каждый выстрел/удар исключает монотонность.
   - **Organic XP Escalator**: логарифмический подъем тона при серийном сборе опыта.
2. **Мобильный мастеринг**:
   - High-Pass фильтр (45 Гц) срезает неслышимый саб-гул, разгружая лимитер.
   - Notch/Peaking фильтр (3.2 кГц, -2.5 dB) сглаживает резонанс динамиков смартфонов.
   - `DynamicsCompressorNode` выступает финальным лимитером.
3. **Platform Haptics**:
   - `hapticImpact('heavy')`: удары Мега-Башмака, ядерные взрывы и детонация камикадзе.
   - `hapticImpact('medium')`: хлесткие удары Кнута, критический урон.
   - `hapticSelection()`: сбор кристаллов XP и капель GOO.
   - `hapticNotification('success')`: получение сундука мутации / эволюция.
